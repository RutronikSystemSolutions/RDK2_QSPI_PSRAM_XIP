...
The PSRAM is additionally tested before entering XIP mode. Testing is performed by writing to and reading from memory using SMIF PDL functions. If the data comparison succeeds, the XIP mode is entered, and the write/read/compare process in XIP mode starts and continues indefinitely at full speed, using the full 8MB of PSRAM memory. If an error is found during the data comparison, it will be printed to the terminal.

<img src="images/psram_example_terminal.png" style="zoom:100%;" />

The QSPI interface is configured using the HAL library, and the SMIF is configured and initiated using the PDL libraries in the function:
